#!/usr/bin/env python3
"""Run a 15-minute OpenClaw Desktop web UI power-user smoke test.

Tests chat, sidebar/project rail, inspector/activity, switching, scroll/history,
and request/console stability. Uses the globally installed Playwright Node module.
"""
import subprocess, pathlib, textwrap, sys
run_dir = pathlib.Path(__file__).resolve().parent
js_path = run_dir / "power_user_smoke.js"
...
(async()=>{
  ...
  await page.evaluate(() => {
    localStorage.setItem('openclaw.middleware.url', 'http://127.0.0.1:8790'); localStorage.setItem('openclaw.middleware.token','devtest');
...
''')
result = subprocess.run(["node", str(js_path)], cwd=run_dir.parents[2])
