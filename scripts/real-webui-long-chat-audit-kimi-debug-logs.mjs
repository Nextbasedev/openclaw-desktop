...
// Enable scroll debug logging and capture console messages
await page.goto(url, { waitUntil: 'networkidle', timeout: 60000 })
await page.evaluate(() => {
  window.localStorage.setItem('openclaw.chat.scroll.debug', '1')
})
...
const metrics = await page.evaluate(async () => {
  ...
  observer.disconnect()
  return { ...state, startScrollTop: container.scrollTop, finalScrollTop: container.scrollTop, finalScrollHeight: container.scrollHeight }
})

const report = {
  ...metrics,
  consoleLogs,
  verdict: metrics.jumpCount === 0 && metrics.textDisappearances === 0 ? 'PASS' : 'ISSUE_FOUND',
}
writeFileSync(join(OUT, 'audit-report-debug.json'), JSON.stringify(report, null, 2))
