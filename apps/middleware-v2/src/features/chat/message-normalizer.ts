...
export function readOpenClawMessageId(message: OpenClawMessage): string | null {
  const id = message.__openclaw?.id;
  return typeof id === "string" && id.trim() ? id.trim() : null;
}

export function normalizeHistoryMessages(sessionKey: string, messages: unknown[], nowMs = Date.now()): ProjectedMessage[] {
  return messages
    .filter((message): message is OpenClawMessage => Boolean(message) && typeof message === "object" && !Array.isArray(message))
    .map((message, index) => ({
      sessionKey,
      openclawSeq: readOpenClawSeq(message, index + 1),
