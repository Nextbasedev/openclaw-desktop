...
async function main() {
  ...
  const cronActivity = await req('/api/commands/middleware_cron_recent_activity', { method: 'POST', body: JSON.stringify({ input: {} }) })
  assert(Array.isArray(cronActivity.events) && Array.isArray(cronActivity.activity), 'cron activity ui and legacy shape')
  ...
  const modelsResponse = await req('/api/commands/middleware_models_list', { method: 'POST', body: JSON.stringify({ input: {} }) })
  assert(Array.isArray(modelsResponse.models), 'models list')
  assert('currentModel' in modelsResponse, 'models currentModel shape')
  ...
  const flowResponse = await req('/api/commands/middleware_onboarding_flow', { method: 'POST', body: JSON.stringify({ input: {} }) })
  assert(flowResponse.flow && flowResponse.state, 'onboarding flow ui shape')
  const botNameResponse = await req('/api/commands/middleware_openclaw_bot_name_get', { method: 'POST', body: JSON.stringify({ input: {} }) })
  assert('botName' in botNameResponse, 'bot name ui shape')
  const modelContractResponse = await req('/api/commands/middleware_onboarding_model_contract', { method: 'POST', body: JSON.stringify({ input: {} }) })
  assert(modelContractResponse.contract?.types?.payloadShape?.modelRef?.options, 'model contract ui shape')
  ...
  const memoryListResponse = await req('/api/commands/middleware_memory_list', { method: 'POST', body: JSON.stringify({ input: {} }) })
  assert(Array.isArray(memoryListResponse.documents) && Array.isArray(memoryListResponse.files), 'memory list ui and legacy shape')
  const memoryRecallResponse = await req('/api/commands/middleware_memory_recall', { method: 'POST', body: JSON.stringify({ input: {} }) })
  assert(Array.isArray(memoryRecallResponse.entries) && Array.isArray(memoryRecallResponse.results), 'memory recall ui and legacy shape')
