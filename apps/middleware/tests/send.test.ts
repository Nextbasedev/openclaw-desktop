...
describe("chat send routes", () => {
  ...
  test("drops Gateway user echo that replays the attached-file prompt body", async () => {
    const app = await createApp(config("text-attachment-user-echo"));
    const context = contextOf(app);
    let historyLoaded = false;
    vi.spyOn(context.gateway, "request").mockImplementation(async (method: string, params?: Record<string, unknown>) => {
      if (method === "chat.send") {
      ...
      if (method === "chat.history") {
        ...
        return {
          ...
          messages: [
            {
              role: "user",
              content: String(params?.message ?? "read this file\n\n<attached-file name=\"hyy.md\" mime=\"text/markdown\">\n# Private file body\n</attached-file>"),
              __openclaw: { id: "gateway-user-echo", seq: 1 },
            },
            { role: "assistant", text: "I read it.", __openclaw: { id: "assistant-1", seq: 2 } },
          ],
        };
      }
      return { ok: true };
    ...
    const bootstrap = await app.inject({ method: "GET", url: "/api/chat/bootstrap?sessionKey=s1" });
    expect(bootstrap.statusCode).toBe(200);
    const messages = bootstrap.json().messages as Array<{ role: string; text?: string; attachments?: unknown[] }>;
    expect(messages).toHaveLength(2);
    expect(messages[0]).toMatchObject({
      role: "user",
      text: "read this file",
      attachments: [{ name: "hyy.md", mimeType: "text/markdown", content: "# Private file body", size: 19 }],
    });
    expect(messages[1]).toMatchObject({ role: "assistant", text: "I read it." });
  ...
  test("live chat delta treats repeated cumulative content as replacement instead of duplicate delta", async () => {
    const app = await createApp(config("live-chat-cumulative-content"));
    const context = contextOf(app);
    const patches: Array<{ type: string; payload?: Record<string, unknown> }> = [];
    vi.spyOn(context.patchBus, "broadcast").mockImplementation((patch) => { patches.push(patch as typeof patches[number]); });
    vi.spyOn(context.gateway, "request").mockImplementation(async (method: string) => {
    ...
    const textPatches = patches.filter((patch) => patch.type === "chat.message.upsert" && patch.payload?.semanticType === "chat.assistant.delta");
    expect(textPatches.map((patch) => (patch.payload?.message as { text?: string } | undefined)?.text)).toEqual([
      "Hello Krish 👋",
      "Hello Krish 👋 here.",
    ]);
    await app.close();
  ...
  test("forwards stable idempotencyKey and keeps execPolicy out of Gateway chat.send", async () => {
    ...
    const res = await app.inject({
      method: "POST",
      url: "/api/chat/send",
      payload: { sessionKey: "s1", text: "hello", idempotencyKey: "stable-key", execPolicy: { security: "allowlist", ask: "on-miss" } },
    });
    expect(res.statusCode).toBe(200);
    ...
    expect(sendCall?.params).not.toHaveProperty("execPolicy");
    expect(calls).toEqual(expect.arrayContaining([
      expect.objectContaining({ method: "sessions.patch", params: expect.objectContaining({ key: "s1", execSecurity: "allowlist", execAsk: "on-miss" }) }),
    ]));
    await app.close();
  });
