...
function AppShell({
  ...
  useEffect(() => {
    const flushLatestMeaningfulLayout = (closed = false) => {
      const snapshot = latestMeaningfulLayoutRef.current
      if (snapshot) void saveWorkspaceLayoutSnapshot(snapshot, { closed }).catch(() => {})
    }
    const onPageHide = () => flushLatestMeaningfulLayout(true)
    const onBeforeUnload = () => flushLatestMeaningfulLayout(true)
    const onVisibilityChange = () => {
      if (document.visibilityState === "hidden") flushLatestMeaningfulLayout(false)
    }
