...
function StatItem({ label, value }: { label: string; value: string | number }) {
  return (
    <div className="flex flex-col items-center rounded-lg bg-muted/20 px-4 py-3">
...
function SkillAvatar({ slug }: { slug: string }) {
  const letter = (slug[0] ?? "S").toUpperCase()
  const hue = slug.split("").reduce((a, c) => a + c.charCodeAt(0), 0) % 360
  return (
    <div
      className="flex size-14 shrink-0 items-center justify-center rounded-xl text-[20px] font-bold text-white"
