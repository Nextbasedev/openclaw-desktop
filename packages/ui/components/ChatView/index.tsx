...
import { canRunSlashCommandWhileGenerating, getSlashCommandName, isStopSlashCommand } from "@/lib/controlSlashCommands"
...
export function ChatView({
  ...
  async function handleSend(payload: ChatComposerSubmit) {
    ...
    const slashCanRunNow = !hasAttachments && !payload.replyTo && canRunSlashCommandWhileGenerating(text, [])
    ...
    const canBypassQueue = Boolean((payload.runWhileGenerating || slashCanRunNow) && !activeRunIsSlashCommand)
      ...
      >
        {state.error ? (
          ...
          <div ref={scrollContentRef} className="min-h-full py-6">
            ...
            <div
              style={{
                height: queuedMessages.length > 0
                  ? Math.min(272, 64 + queuedMessages.length * 48)
                  : 24,
              }}
            />
          </div>
