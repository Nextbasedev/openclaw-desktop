...
export function ToolCallDetails({
  ...
  const fetchFullResult = useCallback(async () => {
    ...
    try {
      ...
      // Keep the inline preview if the middleware cannot recover the full result.
    } finally {
      setFetching(false)
    }
  ...
  const canFetchFullOutput = Boolean(outputText.includes("(truncated)") && sessionKey && call.id)
          ...
          <div className="transition-all duration-300 ease-out animate-in fade-in-0 slide-in-from-top-1">
            {effectiveOutputText ? (
              <>
                ...
                {(fullOutputText || canFetchFullOutput) && (
                  <button
                    type="button"
                    onClick={() => {
                  ...
                  >
                    {fetching
                      ...
                      : showFull
                        ? "Collapse output"
                        : effectiveFullText
                          ? `Show full output (${Math.round(effectiveFullText.length / 1024)}KB)`
                          : "Load full output"}
                  </button>
                )}
              </>
