...
const SPACE_DOT_GRADIENTS = [
  ...
  "from-slate-300 via-zinc-400 to-neutral-500",
]
...
export function SpacesSection({ spaces, activeSpaceId, onSwitch, onCreate }: Props) {
  ...
  return (
    <div className="relative z-10 border-t border-white/[0.06] px-3 pb-3 pt-2.5 dark:border-white/[0.06]">
      <div
        className={cn(
          "flex h-9 items-center justify-between gap-2 rounded-[14px] px-2.5",
        ...
        <div className="flex min-w-0 items-center gap-3">
          {activeSpace && (
            ...
            >
              ...
              <span className="absolute inset-[3px] rounded-full bg-white/24" />
              <span className="relative size-1.5 rounded-full bg-white/75" />
            </button>
          )}
            ...
            {spaces.slice(0, 5).map((space, index) => {
              ...
              return (
                <button
                  ...
                  className="group flex size-2.5 shrink-0 cursor-pointer items-center justify-center rounded-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/20"
                >
                  <span
                    className={cn(
                      ...
                      SPACE_DOT_GRADIENTS[index % SPACE_DOT_GRADIENTS.length],
                    )}
                  />
                </button>
              )
