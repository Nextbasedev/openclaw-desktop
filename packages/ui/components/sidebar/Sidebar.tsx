...
export function Sidebar({
  ...
  const handleCreateProject = useCallback(async () => {
    ...
    try {
      ...
      const defaultTopic = await createTopic({
      ...
      setSelectedTopicId(defaultTopic.topic.id)
      onProjectSelect?.(result.project.id)
      onProjectNameSelect?.(result.project.name)
      onTopicSelect?.(defaultTopic.topic.id)
      onTopicNameSelect?.(defaultTopic.topic.name)
