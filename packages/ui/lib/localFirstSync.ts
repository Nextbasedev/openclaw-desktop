...
function messageKey(sessionKey: string) {
  return `local:first:session:${sessionKey}:messages`
}
function statusKey(sessionKey: string) {
  return `local:first:session:${sessionKey}:status`
}
...
function messageIdOf(message: ChatMessage) {
  return (
    message.messageId ||
...
function mergeMessages(existing: ChatMessage[], incoming: ChatMessage[]) {
  ...
  for (const message of dedupeChatMessages(existing)) {
    byId.set(messageIdOf(message), message)
  }
  for (const message of dedupeChatMessages(incoming)) {
    const key = messageIdOf(message)
    const prev = byId.get(key)
    byId.set(
      key,
      prev ? { ...prev, ...message, text: message.text || prev.text } : message
    )
  }
  return dedupeChatMessages([...byId.values()]).sort((a, b) => {
...
bc?.addEventListener("message", (event: MessageEvent<LocalSyncEvent>) => {
  ...
  if (data.type === "messages")
    emitSet(messageListeners.get(data.state.sessionKey), data.state)
...
export async function localSyncGetMessages(sessionKey: string) {
  const state = await persistentCacheGet<LocalMessageState>(
    messageKey(sessionKey)
  )
  if (state) return state
  const messages = await persistentCacheGet<ChatMessage[]>(
    `session:${sessionKey}:uiMessages`
  )
  const status = await persistentCacheGet<StreamStatus>(statusKey(sessionKey))
  return messages?.length
    ? { sessionKey, messages, status, updatedAt: now() }
    : null
}

export async function localSyncSetMessages(
  sessionKey: string,
  messages: ChatMessage[],
  status?: StreamStatus | null,
  updatedAt = now()
) {
  const existing = await persistentCacheGet<LocalMessageState>(
    messageKey(sessionKey)
  )
  const merged = mergeMessages(existing?.messages ?? [], messages).slice(-500)
  const state: LocalMessageState = {
    sessionKey,
    messages: merged,
    status: status ?? existing?.status ?? null,
    updatedAt,
  }
  await persistentCacheSet(messageKey(sessionKey), state, {
    ttlMs: CACHE_TTL_MS,
  })
  ...
  if (state.status)
    await persistentCacheSet(statusKey(sessionKey), state.status, {
      ttlMs: CACHE_TTL_MS,
    })
...
export async function localSyncSetStatus(
  sessionKey: string,
  status: StreamStatus
) {
...
export function localSyncSubscribeMessages(
  sessionKey: string,
  listener: Listener<LocalMessageState>
) {
  const set =
    messageListeners.get(sessionKey) ?? new Set<Listener<LocalMessageState>>()
  set.add(listener)
