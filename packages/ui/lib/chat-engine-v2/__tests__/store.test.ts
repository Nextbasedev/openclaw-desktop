...
describe("abort suppression", () => {
  test("suppresses late assistant and tool patches after local abort", () => {
    ...
    const state = getGlobalChatSession("abort-s1")
  ...
  test("keeps aborted assistant answer suppressed after the next user turn", () => {
    seedGlobalChatSession({
      sessionKey: "abort-s2",
      messages: [
        { messageId: "u1", role: "user", text: "describe yourself" },
    ...
    expect(getGlobalChatSession("abort-s2")?.messages).not.toEqual(expect.arrayContaining([
    ...
    expect(getGlobalChatSession("abort-s2")?.messages).toEqual(expect.arrayContaining([
