...
describe("applyChatPatch", () => {
  ...
  test("anchors live assistant patches after the latest user when runId is missing", () => {
    ...
    state = applyChatPatch(state, {
      ...
      patch: {
        ...
        payload: {
          ...
          messageId: "user-2",
          messageSeq: 2,
          message: { role: "user", text: "second prompt", __openclaw: { id: "user-2", seq: 2 } },
        },
        createdAtMs: 2,
      },
    })
    ...
    const latestUser = withAssistant.messages.find((m) => m.messageId === "user-2")
    const assistant = withAssistant.messages.find((m) => m.messageId === "assistant-2")
    expect(latestUser?.gatewayIndex).toBe(2)
    expect(assistant?.gatewayIndex).toBe(3)
  })
