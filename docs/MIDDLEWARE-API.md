# OCPlatform Desktop — Middleware API Reference

> **Audience:** Frontend developers building against the middleware HTTP/WS layer.
> **Generated from source:** `apps/middleware/src/` (schema version 2, projection version 3)
...
```
┌──────────────────────┐      HTTP / WS        ┌─────────────────────┐      WebSocket       ┌──────────────────┐
│  Desktop Frontend    │ ◄──────────────────► │  Middleware      │ ◄──────────────────► │ OCPlatform Gateway │
...
|----------|---------|-------------|
| `MIDDLEWARE_HOST` (or `HOST`) | `127.0.0.1` | Bind address |
| `MIDDLEWARE_PORT` (or `PORT`) | `8787` | Listen port |
| `MIDDLEWARE_DB` | `~/.openclaw/middleware/state.sqlite` | SQLite database path |
