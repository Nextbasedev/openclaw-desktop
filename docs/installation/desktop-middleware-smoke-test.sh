...
CRON_STREAM_SAMPLE="$(timeout 3s curl -fsS -N "$BASE_URL/api/stream/cron" -H "Authorization: Bearer $TOKEN" 2>/dev/null | head -n 5 || true)"
if ! grep -Eq 'cron stream ready|cron\.ready' <<<"$CRON_STREAM_SAMPLE"; then
