# Middleware Frontend Integration Guide

This guide documents the current `apps/middleware` HTTP/WebSocket surface for frontend engineers. It is based on the source in `apps/middleware/src` and the v2 frontend client in `packages/ui/lib/chat-engine-v2`.

Middleware is not the legacy desktop middleware on port `8787`. Its default base URL is:

```text
http://127.0.0.1:8787
```

Frontend code should treat this URL as the canonical app backend for middleware chat projection and patch streaming. Do not mix v2 chat state with legacy `/api/stream/chat/:sessionKey` or legacy `8787` session status heuristics unless a feature is explicitly still missing from v2.

## 1. Current scope and caveats

Middleware currently implements:
...
Middleware currently does **not** implement first-class endpoints for these legacy/backend features:
...
For those missing features, the current repo still has legacy/shared API contracts under `packages/shared/src/api/*` and legacy frontend helpers that talk to port `8787` or remote middleware (`packages/ui/lib/middleware-client.ts`, `packages/ui/lib/ipc.ts`, `packages/ui/components/inspector/workspace-api.ts`). Treat these as caveats, not v2 contracts. If the goal is “v2 only”, these endpoints must be added to `apps/middleware/src` before frontend code can use them on port `8787`.

## 2. Auth and configuration assumptions

Middleware uses CORS with `origin: true` and `credentials: false`. The current v2 HTTP client does not send an Authorization header.

Server environment:

- `MIDDLEWARE_HOST` or `HOST`: defaults to `127.0.0.1`.
- `MIDDLEWARE_PORT` or `PORT`: defaults to `8787`.
- `MIDDLEWARE_DB`: defaults to `~/.openclaw/middleware/state.sqlite`.
...
- Default v2 URL: `http://127.0.0.1:8787`.
- Environment override: `NEXT_PUBLIC_MIDDLEWARE_V2_URL`.
- Browser localStorage override: `openclaw.middleware.v2.url`.
- If the browser is loaded from a non-loopback host and the v2 URL is loopback, the client rewrites the host to the browser hostname and preserves port `8787`.
...
```bash
curl -s http://127.0.0.1:8787/api/system/info | jq
```
...
```json
{
  ...
  "host": "127.0.0.1",
  "port": 8787,
  "databasePath": "/home/user/.openclaw/middleware/state.sqlite",
  "gatewayUrl": "ws://127.0.0.1:18789",
  "uptimeMs": 12345
}
```
...
```bash
curl -s http://127.0.0.1:8787/api/gateway/status | jq
curl -s -X POST http://127.0.0.1:8787/api/gateway/reconnect | jq
```
...
```bash
curl -s http://127.0.0.1:8787/api/diagnostics | jq
curl -s http://127.0.0.1:8787/api/diagnostics/patch-clients | jq
```
...
Middleware chat is projection-based:
...
```bash
curl -s 'http://127.0.0.1:8787/api/chat/bootstrap?sessionKey=s1&limit=200' | jq
```
...
```bash
curl -s 'http://127.0.0.1:8787/api/chat/messages?sessionKey=s1&afterSeq=0&limit=100' | jq
```
...
```bash
curl -s -X POST http://127.0.0.1:8787/api/chat/abort \
  -H 'Content-Type: application/json' \
...
```bash
curl -s -X POST http://127.0.0.1:8787/api/exec/approval/resolve \
  -H 'Content-Type: application/json' \
...
```bash
curl -s 'http://127.0.0.1:8787/api/patches?afterCursor=42&limit=1000' | jq
```
...
```text
ws://127.0.0.1:8787/api/stream/ws?afterCursor=42
```

Browser example:

```ts
const url = new URL('http://127.0.0.1:8787/api/stream/ws')
...
Run statuses from `apps/middleware/src/features/chat/repo.runs.ts`:
...
Current v2 status: **not implemented on port 8787**.

The shared legacy contract defines terminal endpoints in `packages/shared/src/api/terminal.ts`, but `apps/middleware/src/app.ts` does not register terminal routes.

Legacy/shared endpoint shapes that still need v2 implementation:

```bash
# Create terminal - not currently available on 8787
curl -s -X POST http://127.0.0.1:8787/api/terminal \
  -H 'Content-Type: application/json' \
  -d '{"projectId":"project-1","topicId":"topic-1","cwd":"/repo","title":"Shell"}'

# Write - not currently available on 8787
curl -s -X POST http://127.0.0.1:8787/api/terminal/terminal-1/write \
  -H 'Content-Type: application/json' \
  -d '{"sessionId":"terminal-1","data":"ls\n"}'

# Resize - not currently available on 8787
curl -s -X POST http://127.0.0.1:8787/api/terminal/terminal-1/resize \
  -H 'Content-Type: application/json' \
  -d '{"sessionId":"terminal-1","cols":120,"rows":32}'

# Close - not currently available on 8787
curl -s -X POST http://127.0.0.1:8787/api/terminal/terminal-1/close \
  -H 'Content-Type: application/json' \
  -d '{"sessionId":"terminal-1"}'

# List - not currently available on 8787
curl -s 'http://127.0.0.1:8787/api/terminal?projectId=project-1'
```
...
- Do not call terminal endpoints on `8787` until routes exist in `apps/middleware/src`.
...
But `apps/middleware/src` currently registers only system, gateway, diagnostics, chat, and patches.

Shared file endpoint shapes that still need v2 implementation:

```bash
# File tree - not currently available on 8787
curl -s 'http://127.0.0.1:8787/api/files/tree?projectId=project-1&path=/'

# Read file - not currently available on 8787
curl -s 'http://127.0.0.1:8787/api/files/read?projectId=project-1&path=README.md'

# Write file - not currently available on 8787
curl -s -X POST http://127.0.0.1:8787/api/files/write \
  -H 'Content-Type: application/json' \
  -d '{"projectId":"project-1","path":"README.md","content":"hello"}'

# Search - not currently available on 8787
curl -s 'http://127.0.0.1:8787/api/files/search?projectId=project-1&query=middleware'
```
...
Those paths are also not present in middleware today.
...
```bash
# Status - not currently available on 8787
curl -s 'http://127.0.0.1:8787/api/git/status?projectId=project-1'

# Diff - not currently available on 8787
curl -s 'http://127.0.0.1:8787/api/git/diff?projectId=project-1&refA=HEAD'

# Branches - not currently available on 8787
curl -s 'http://127.0.0.1:8787/api/git/branches?projectId=project-1'

# Switch branch - not currently available on 8787
curl -s -X POST http://127.0.0.1:8787/api/git/switch-branch \
  -H 'Content-Type: application/json' \
  -d '{"projectId":"project-1","branchName":"feature/x","create":false}'

# Commit - not currently available on 8787
curl -s -X POST http://127.0.0.1:8787/api/git/commit \
  -H 'Content-Type: application/json' \
...
- Add a dedicated v2 Git feature module before using `8787` for Git.
...
```bash
# Activity feed - not currently available on 8787
curl -s 'http://127.0.0.1:8787/api/activity/feed?projectId=project-1'

# Agents tree - not currently available on 8787
curl -s 'http://127.0.0.1:8787/api/agents/tree?projectId=project-1'

# Processes - not currently available on 8787
curl -s 'http://127.0.0.1:8787/api/processes?projectId=project-1'
```
...
Settings/model/pin related shared contracts exist elsewhere but are not registered in middleware:
...
- Add explicit v2 endpoints for settings/models/pins before depending on them through `8787`.

## 10. Frontend implementation checklist

Use this as the v2 chat integration checklist:

- Configure the v2 base URL from `openclaw.middleware.v2.url`, `NEXT_PUBLIC_MIDDLEWARE_V2_URL`, or default `http://127.0.0.1:8787`.
- Health-check `/health` and confirm `service === "openclaw-middleware"`.
...
```bash
curl -s http://127.0.0.1:8787/health | jq '.service,.port'
```
...
```bash
curl -s 'http://127.0.0.1:8787/api/chat/bootstrap?sessionKey=s1&limit=200' | jq '{runStatus,statusLabel,activeRun,cursor,sessionStatus,tools,toolCalls}'
curl -s 'http://127.0.0.1:8787/api/patches?afterCursor=0&limit=20' | jq '.patches[-10:]'
curl -s http://127.0.0.1:8787/api/diagnostics | jq
```
...
```bash
curl -s 'http://127.0.0.1:8787/api/patches?afterCursor=0&limit=1000' \
...
localStorage.setItem('openclaw.middleware.v2.url', 'http://127.0.0.1:8787')
```
...
Do not confuse the two. Legacy `openclaw.middleware.url` usually points to `8787`; v2 chat uses `openclaw.middleware.v2.url` and `8787`.

### Port quick reference

- `8787`: middleware HTTP/WebSocket backend documented here.
- `18789`: OpenClaw Gateway WebSocket used internally by middleware.
- `8787`: legacy desktop middleware/server used by older UI features.

## 12. Endpoint inventory from `apps/middleware/src`

Registered in `apps/middleware/src/app.ts`:
