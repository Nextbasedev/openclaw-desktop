...
| 0018 | [0018-multiturn-ordering-and-tool-awaiting.md](commits/0018-multiturn-ordering-and-tool-awaiting.md) | Fix multi-turn row ordering (`maxSeq` never advanced to server seq high-water mark → 2nd-turn optimistic user msg sorted above 1st assistant reply; add `noteSeq`) + stale tool `awaitingResult` (`?? prev` kept `true` after real result → card stuck "waiting" under DONE). +4 tests, 30/30. Middleware arg-descriptor + result-backfill still open |
| 0019 | [0019-golden-corpus-and-tool-args-descriptor-fix.md](commits/0019-golden-corpus-and-tool-args-descriptor-fix.md) | Capture REAL `/api/patches` tool-call stream as golden JSONL + test against it. Middleware fix: stop projecting the gateway item descriptor `{title,itemId,kind}` as tool args (it clobbered real argsMeta via COALESCE → wrong ARGUMENTS). Golden replay proves bug-2 awaiting-clear against real bytes. UI 33/33, mw 184/184, builds green |
| 0020 | [0020-cross-session-bleed-fix.md](commits/0020-cross-session-bleed-fix.md) | Fix cross-session message bleed: `/api/stream/ws` is a GLOBAL feed (no server-side session scoping); neither ChatSyncClient nor applyPatch filtered by sessionKey → other sessions' messages appeared in the open chat. Filter foreign frames at both layers, still advancing the global cursor. +3 tests, 36/36 |
| 0021 | [0021-run-lifecycle-authoritative-per-patch.md](commits/0021-run-lifecycle-authoritative-per-patch.md) | KEYSTONE: make run lifecycle authoritative on EVERY patch. Wire embeds runStatus+activeRun in every frame and ships the success terminal inside chat.assistant.final (no chat.run.done) → live runs never finalized (Composer stuck 'Stop', live multi-turn = all users then all assistants). `reconcileRunState` after every content handler. Golden s01/s03 regression (real streams, no run.done). 38/38 |
| 0022 | [0022-remove-per-session-gap-guard.md](commits/0022-remove-per-session-gap-guard.md) | Remove the store's per-session gap guard. Post-0020 the store consumes a session-filtered substream of the GLOBAL cursor, so forward jumps are normal (a running subagent advances the cursor); the old guard spuriously re-bootstrapped. Gap recovery stays in ChatSyncClient (contiguous global). |
| 0023 | [0023-subagent-rendering-and-run-state-guard.md](commits/0023-subagent-rendering-and-run-state-guard.md) | First-class sub-agent (sessions_spawn) rendering: SubagentRow projection + handleSubagent + SubagentCard (was raw tool JSON). + CRITICAL: reconcileRunState ignores chat.subagent.* (idle snapshot would clear parent activeRun mid-run). Golden regression from real stream. 42/42 |
